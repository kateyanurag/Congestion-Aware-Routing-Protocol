...
void sendPackets(int32_t fd, int32_t ifindex, uint16_t SrcMAC1, uint32_t SrcMAC2,
		 uint16_t DestMAC1, uint32_t DestMAC2, uint16_t type, uint32_t src, uint32_t dest, uint32_t port, uint32_t vlanTag, int32_t *count) {
  int32_t i;
...
void sendReceive(int32_t fd, int32_t ifindex, uint16_t SrcMAC1, uint32_t SrcMAC2,
		 uint16_t DestMAC1, uint32_t DestMAC2, uint16_t type, uint32_t src, uint32_t dest, uint32_t port, uint16_t vlanID, uint32_t flow) {
  unsigned char buf[MAX_PACKET_SIZE];
  int32_t sendCount = 0;
  ...
  uint32_t vlanTag = 0x81000000 | vlanID;
	...
	flow = 3;
	for(i = 0; i < flow; i++){
		sendPackets(fd, ifindex, SrcMAC1, SrcMAC2, DestMAC1, DestMAC2, type, src, dest, port, vlanTag, &sendCount);
	}
...
/*  for (; sendCount < MaxCommCount && receiveCount < MaxCommCount;) {
	...
	sendPackets(fd, ifindex, SrcMAC1, SrcMAC2, DestMAC1, DestMAC2, type, src, dest, port, vlanTag, flow, &sendCount);
	lastTime = currTime;
      }
    }
...
int32_t transmit_packets(int32_t src, int32_t dest, int32_t port, int32_t vlanid, int32_t flow) {
  int32_t ifindex;
  int32_t myTermNum = 0;
...
*/
  ...
  sendReceive(fd, ifindex, SrcMAC1, SrcMAC2, DestMAC1, DestMAC2, ETH_P_Exp, src, dest, port, (int16_t)vlanid, flow);
	return 0;
}
