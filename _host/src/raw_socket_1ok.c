...
//  strncpy(ifname, IFNAME, IFNAMSIZ);
  ...
  strcat((char*)str+1, (char *)interface_name);
...
//  if (DEBUG) 
	printf("\n %s opened (fd=%d interface=%d)", ifname, fd, ifindex);
...
ssize_t createPacket(EtherPacket *packet, uint16_t destMAC1, uint32_t destMAC2,
	...
	//if(DEBUG) 	
	printf("\npayload length=%d",(int)strlen(payload));
...
//ssize_t packetSize = sizeof(EtherPacket);	
  ...
  packet->type = htons(type);
	...
	/***********************************************************************/
	//if (DEBUG) 
printf("\n After adding ethernet_header: packetSize=%d",(int)packetSize);
...
/*	char *dIest_ip = "10.0.0.";
	char str[3];	
	sprintf((char *)str, "%d", dest);
	if(dest > 9)
		str[2] = '\0';
	else
		str[1] = '\0';
...
/*	char dest_ip[10] = {'1', '0', '.', '0', '.', '0' , '.', '\0'};
       ...
       //     puts(str);
         //   printf("\n Len:%d", (int)strlen(str));
      //    char interface_name []= {'-', 'e', 't', 'h', '0'};
         strcat((char *)dest_ip, (char*)str);
//          printf("\n IP : %s", dest_ip);
	
	packet->iph_destip = inet_addr((char*)dest_ip);
*/	
	...
	*/
	//if (DEBUG) 
	printf("\nAfter adding ip_header: packetSize=%d",(int)packetSize);
	
	//struct udphdr *udph = (struct udphdr *) (payload + sizeof(struct iphdr) + sizeof(EtherPacket));
	/* UDP Header */

	packet->udph_srcport = htons(SRC_PORT);
	packet->udph_destport = htons(DST_PORT); // global var
	packet->udph_chksum = 0; // skip
	//if (DEBUG) 
	printf("\nAfter adding udp_header: packetSize=%d",(int)packetSize);
...
void sendPackets(int32_t fd, int32_t ifindex, uint16_t SrcMAC1, uint32_t SrcMAC2,
 ...
 printPacket((EtherPacket*)packet, packetSize, "net/util/m h1 sudo ./sendrecv 0 1 0 1Sent:    ");
  if (sizeout < 0) {
    perror("sendto");
  } else {
    //if (DEBUG) {
      printf("\n %d bytes sent through interface (ifindex) %d\n",(int32_t)sizeout, (int32_t)ifindex);
    //}
  }
}
...
int32_t transmit_packets(int32_t source, int32_t destination, int32_t portno, int32_t flow_chunk, int16_t vlan_id) {
  int32_t ifindex;
  int32_t myTermNum = 0;
  ...
  int32_t i;
	...
	flow = flow_chunk;
	vlanID = vlan_id;
	printf("\n ++++++++++++++++++++++ SRC: %d, DEST: %d, Port:%d, Flow: %d, VLAN:%d ", src, dest, port, flow, vlan_id);
